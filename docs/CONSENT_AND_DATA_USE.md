...
|---|---|---|---|
| **OpenAI** (ChatGPT/Codex subscription) | During each agent run: the agent's instructions, your persona file, your purchase profile, the task list, and the content of the amazon.in pages the agent reads | This is what makes your agent run; Hermes authenticates through OpenAI device-code OAuth using your ChatGPT/Codex account, not an API key | Processing and retention follow the signed-in ChatGPT/Codex plan and its current data controls. Review the account's privacy/data-control settings and the current OpenAI documentation before the course begins. |
